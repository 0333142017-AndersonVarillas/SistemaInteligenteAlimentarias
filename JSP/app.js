...
function generarResumenTexto() {
    const blocks = ['bloque1', 'bloque2', 'bloque3', 'bloque4'];
    const allKeys = Object.keys(localStorage);
    let texto = 'Resumen de resultados. ';
    let globalSum = 0;
    let count = 0;
    const blockMax = 5;
    ...
    blocks.forEach((b, i) => {
        ...
        const values = Object.values(data);
        const total = values.reduce((a, b) => a + b, 0);
        const pct = (total / blockMax) * 100;
        globalSum += total;
        count += blockMax;

        texto += `Bloque ${i + 1}: ${total} de ${blockMax} respuestas positivas, equivalente a ${pct.toFixed(1)} por ciento. `;
        if (pct <= 40)
    ...
    const overallPct = (globalSum / count) * 100;
    let estado = '';
    if (overallPct <= 40)
        estado = '🔴 Alto riesgo general.';
    else if (overallPct <= 70)
        estado = '🟡 Riesgo medio general.';
    else
        estado = '🟢 Buen estado general.';

    texto += `Resultado global: ${overallPct.toFixed(1)} por ciento. ${estado}`;
    return texto;
}

// Consejos coherentes con los resultados
function generarConsejos() {
    const blocks = ['bloque1', 'bloque2', 'bloque3', 'bloque4'];
    const allKeys = Object.keys(localStorage);
    const blockMax = 5;
    let consejos = 'Recomendaciones específicas: ';
    ...
    blocks.forEach((b, i) => {
        ...
        consejos += `Para el bloque ${i + 1}, `;
        if (pct <= 40)
            consejos += 'urge tomar acciones inmediatas para corregir fallas críticas. ';
        else if (pct <= 70)
            consejos += 'deberías reforzar procedimientos y capacitación del personal. ';
        else
            consejos += 'mantén el nivel actual de cumplimiento y sigue monitoreando. ';
    });

    return consejos;
}

// === Voz sincronizada con el resumen ===
function leerResultados() {
    ...
    const texto = generarResumenTexto() + ' ' + generarConsejos();
...
window.generarConsejos = generarConsejos;
window.leerResultados = leerResultados;
